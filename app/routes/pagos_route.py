...
def mostrar_pagos():
    ...
    return render_template('views/pagos.html', deudas=deudas)


# recibir datos de nueva deuda que sera aplicada a todos los puestos y registrarla en db
@pagos_bp.route('/crear_deudas', methods=['POST'])
def crear_deudas():
    data = request.json
...
@pagos_bp.route('/registrar_pago/', methods=['POST'])
def registrar_pago():
...
@pagos_bp.route('/reporte/<int:deuda_id>', methods=['GET'])
